...
def test_RO_exact():
    ...
    r = EdgeNormalizer()
    edge = r.resolve_curie('RO:0002506')
...
def test_exact_slot_URI_non_RO():
    ...
    r = EdgeNormalizer()
    edge = r.resolve_curie('WIKIDATA_PROPERTY:P2293')
    assert edge.identifier == 'biolink:genetic_association'
    assert edge.label == 'genetic association'
...
def test_RO_bad():
    ...
    r = EdgeNormalizer()
    edge = r.resolve_curie('RO:0002214') #has prototype
    assert edge.label == 'related to'
